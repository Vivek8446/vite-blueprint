...
try {
  // Step 1: Create Vite Project with compatible version
  spinner.start("Creating Vite project...");
  run(`npm create vite@5.5.4 ${projectName} -- --template ${variant}`);
  spinner.succeed("Vite project created successfully!");

  // Step 2: Install Dependencies with compatible versions
  spinner.start("Installing dependencies...");
  run(`npm i`, { cwd: projectPath });
  
  // Install compatible Vite version first
  run(`npm i vite@^6.0.0`, { cwd: projectPath });
  
  // Then install Tailwind with legacy peer deps to handle any remaining conflicts
  run(`npm i tailwindcss @tailwindcss/vite --legacy-peer-deps`, { cwd: projectPath });
